...
async function saveCursistaSmp(record) {
  ...
  if (!String(record.casamentoDele || '').trim() && !String(record.casamentoDela || '').trim()) {
    throw new Error('A data de casamento religioso de pelo menos um dos cônjuges deve ser preenchida.');
  }
