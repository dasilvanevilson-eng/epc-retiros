...
const studentRegistrationPrintFieldGrid = (record, fields) => `<div class="print-field-grid">${fields.map(([label, key, type]) => `<div class="print-field"><strong>${escapeHtml(label)}</strong><span>${studentRegistrationPrintValue(record?.[key], type)}</span></div>`).join('')}</div>`;
...
function individualStudentRegistrationPrintContent(record) {
  return [
    ...
    studentRegistrationPrintSection('Formação e vivência', studentRegistrationPrintFieldGrid(record, [
      ['É batizado(a)?', 'batizado'], ['Fez primeira comunhão?', 'primeiraComunhao'], ['Estuda?', 'estuda'], ['Série', 'serie'], ['Escola', 'escola'], ['Fez algum retiro?', 'fezRetiro'], ['Qual retiro?', 'qualRetiro'],
    ])),
    studentRegistrationPrintSection('Família e convite', studentRegistrationPrintFieldGrid(record, [
      ['Nome do pai', 'nomePai'], ['Telefone do pai', 'telefonePai', 'phone'], ['Nome da mãe', 'nomeMae'], ['Telefone da mãe', 'telefoneMae', 'phone'], ['Pais participam de movimento?', 'paisMovimento'], ['Qual movimento?', 'qualMovimento'], ['Quem convidou?', 'convidou'], ['Camiseta', 'camiseta'],
    ])),
    studentRegistrationPrintSection('Saúde e cuidados', studentRegistrationPrintFieldGrid(record, [
      ['Intolerância alimentar?', 'intoleranciaAlimentos'], ['Qual intolerância?', 'qualIntolerancia'], ['Alergia a medicamento?', 'alergiaMedicamento'], ['Qual alergia?', 'qualAlergia'], ['Medicamento contínuo?', 'medicamentoContinuo'], ['Qual medicamento contínuo?', 'qualMedicamentoContinuo'], ['Medicamento para dor de cabeça', 'medicamentoCabeca'], ['Medicamento para dor no estômago', 'medicamentoEstomago'],
    ])),
  ].join('');
}
...
async function renderHome({ focusChangedMessage = '' } = {}) {
  ...
  const kidsRows = (rows) => rows.length ? `<div class="student-health-list kids-health-list">${rows.map((kid) => `<div><strong>${escapeHtml(kid.nome || 'Sem nome')}<span class="student-health-inline">${escapeHtml(ageInYearsAndMonths(kid.nascimento))}</span></strong><div class="kids-care-comments"><small>Cadastrada por: ${escapeHtml(kid.volunteer || 'Não informado')}${kid.contact ? ` · Contato: ${escapeHtml(kid.contact)}` : ' · Contato não informado'}</small><small>Origem: Equipe de trabalho · Setor de trabalho: ${escapeHtml(Array.isArray(kid.sectors) && kid.sectors.length ? kid.sectors.join(', ') : 'Não informado')}</small></div></div>`).join('')}</div>` : '<p class="empty-state">Nenhuma criança cadastrada no Espaço Kids.</p>';
...
async function renderNewRetreat(returnHash = '#configuracoes') {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      const retreat = { id: createId(), nome: values.get('nome').trim(), dataInicio: values.get('dataInicio'), dataTermino: values.get('dataTermino'), local: values.get('local').trim(), tipoFichaCursista: values.get('tipoFichaCursista') || defaultStudentFormType, valorInscricaoCursista: parseCurrency(values.get('valorInscricaoCursista')), valorInscricaoVoluntario: parseCurrency(values.get('valorInscricaoVoluntario')), valorFoto: parseCurrency(values.get('valorFoto')), valorCamisetaOficial: parseCurrency(values.get('valorCamisetaOficial')), idadeMaximaEspacoKids: Number(values.get('idadeMaximaEspacoKids')) || 0, numeroPrevistoFichasCursista: normalizeExpectedStudentFileCount(values.get('numeroPrevistoFichasCursista')), setores: sortedSectors, setoresPublicos: sortedSectors, setoresInscricoesEncerradas, dias: serviceDays.length ? serviceDays : [...retreatDefaults.dias], contribuicoes: [...retreatDefaults.contribuicoes], linksSetores: syncSectorLinks({ linksSetores: setoresInscricoesEncerradas.map((setor) => ({ setor, inscricoesEncerradas: true })) }, knownSectors(sortedSectors)), status: 'preparacao', createdAt: new Date().toISOString() };
...
async function renderEditRetreat(id, returnHash = '#configuracoes') {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    const dataInicio = isPublished ? (retreat.dataInicio || '') : values.get('dataInicio');
    const dataTermino = isPublished ? (retreat.dataTermino || '') : values.get('dataTermino');
    ...
    Object.assign(retreat, { nome: values.get('nome').trim(), dataInicio, dataTermino, local: String(values.get('local') || '').trim(), tipoFichaCursista: values.get('tipoFichaCursista') || defaultStudentFormType, valorInscricaoCursista: parseCurrency(values.get('valorInscricaoCursista')), valorInscricaoVoluntario: parseCurrency(values.get('valorInscricaoVoluntario')), valorFoto: parseCurrency(values.get('valorFoto')), valorCamisetaOficial: parseCurrency(values.get('valorCamisetaOficial')), idadeMaximaEspacoKids: Number(values.get('idadeMaximaEspacoKids')) || 0, numeroPrevistoFichasCursista: normalizeExpectedStudentFileCount(values.get('numeroPrevistoFichasCursista')), setores: sortedSectors, setoresPublicos: sortedSectors, setoresInscricoesEncerradas, dias: serviceDays.length ? serviceDays : (retreat.dias?.length ? retreat.dias : [...retreatDefaults.dias]), linksSetores: syncSectorLinks({ ...retreat, linksSetores: existingLinks }, knownSectors(sortedSectors)), updatedAt: new Date().toISOString() });
...
async function renderCrachas() {
  ...
  <section class="panel badge-start-panel" id="badge-start-panel">
    ...
    ${canViewBadgeSectorNames ? '<button type="button" class="badge-start-option" data-badge-view="sector-names"><strong>Setores no crach&aacute;</strong><span>Ajuste o nome do setor para aparecer no crach&aacute;</span></button>' : ''}
