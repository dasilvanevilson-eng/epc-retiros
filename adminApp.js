...
async function renderRecebedor() {
  ...
  const receiverSummaryHtml = `<section class="receiver-summary"><article><span>Já contribuíram</span><strong>${paidPeopleCount}</strong><small>pessoa(s)</small></article><article><span>Falta contribuir</span><strong>${totalPeopleCount - paidPeopleCount}</strong><small>pessoa(s)</small></article><article><span>Total das contribuições</span><strong>${currency(totalPaid)}</strong><small class="receiver-balance-diff">Diferença: <b>${currency(balance)}</b></small></article><article><span>Valor a receber</span><strong>${currency(remaining)}</strong></article></section>`;
...
async function renderPublicForm(id, embedded = false) {
  ...
  mount.innerHTML = `<main class="public-shell"><header class="hero"><div><p class="eyebrow">Equipe de trabalho</p><h1>${escapeHtml(retreat.nome)}</h1><p class="hero-copy">Preencha seus dados para organizarmos sua participação com carinho e antecedência.</p></div></header>${adminSearchPanel}<form id="public-form">${stateDatalist()}
    ...
    <section class="form-section"><div class="section-heading"><span>07</span><div><h2>Informações adicionais</h2><p>Ajude-nos a cuidar bem de você e de sua família.</p></div></div><div class="choice-block"><div class="kids-heading"><h3>Espaço Kids <b>*</b></h3><label><input type="checkbox" name="kidsNotNeeded"> Não necessito do Espaço Kids</label></div><p class="hint kids-hint">Informe o nome de suas crianças que utilizarão o Espaço Kids ou marque que não necessita. Deixe em branco as linhas não utilizadas.</p><div class="kids-list">${kidsFields}</div></div><div class="volunteer-term-topic"><div><h3>Termo de adesão de voluntariado</h3></div><button type="button" id="read-volunteer-term">Ler termo</button></div><div class="fields"><label class="field"><span>Observação</span><textarea name="observacao" rows="4"></textarea></label></div></section>
