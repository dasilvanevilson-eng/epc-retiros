...
async function renderCursista() {
  ...
  const financialSummaryTable = (rows) => {
    ...
    return `<div class="receiver-report-preview student-financial-summary-preview"><table><thead><tr><th>Nome completo do cursista</th><th>Valor da inscrição</th><th>Valor pago</th><th>Saldo a pagar</th></tr></thead><tbody>${rows.map((row) => `<tr><td>${escapeHtml(row.nome)}</td><td>${currency(row.valorInscricao)}</td><td>${currency(row.valorPago)}</td><td>${currency(row.saldoPagar)}</td></tr>`).join('') || '<tr><td colspan="4">Nenhum cursista encontrado.</td></tr>'}</tbody><tfoot><tr><th>Total</th><td>${currency(totals.valorInscricao)}</td><td>${currency(totals.valorPago)}</td><td>${currency(totals.saldoPagar)}</td></tr></tfoot></table></div>`;
  };
  const financialSummaryDocument = (rows) => `<!doctype html><html lang="pt-BR"><head><meta charset="UTF-8"><title>${escapeHtml(financialSummaryTitle)}</title><style>@page{size:A4;margin:10mm}body{margin:0;color:#26382c;font-family:Arial,sans-serif}h1{margin:0 0 6px;font-size:22px}p{margin:0 0 18px;color:#667268}table{width:100%;table-layout:fixed;border-collapse:collapse;font-size:12px}th,td{padding:8px;border:1px solid #d9d1c3;text-align:left;vertical-align:top}th{background:#edf5e9;color:#285130}tfoot th,tfoot td{background:#f6fbf2;font-weight:700}th:first-child,td:first-child{width:auto;overflow-wrap:anywhere;word-break:normal}th:nth-child(2),th:nth-child(3),th:nth-child(4),td:nth-child(2),td:nth-child(3),td:nth-child(4){width:105px;white-space:nowrap;font-weight:700}</style></head><body><h1>${escapeHtml(financialSummaryTitle)}</h1><p>Gerado em ${new Intl.DateTimeFormat('pt-BR', { dateStyle: 'short', timeStyle: 'short' }).format(new Date())}</p>${financialSummaryTable(rows)}</body></html>`;
