...
async function renderRecebedor() {
  ...
  const receiverSummaryHtml = `<section class="receiver-summary"><article><span>Já contribuíram</span><strong>${paidCount}</strong><small>registro(s)</small></article><article><span>Falta contribuir</span><strong>${receiverRows.length - paidCount}</strong><small>registro(s)</small></article><article><span>Total das contribuições</span><strong>${currency(totalPaid)}</strong><small class="receiver-balance-diff">Diferença: <b>${currency(balance)}</b></small></article><article><span>Valor a receber</span><strong>${currency(remaining)}</strong></article></section>`;
...
async function route() {
  ...
  if (target === 'cursista') {
    ...
    const loadStudent = (student) => { selectedStudentId = student.id || ''; editSelectedStudent.hidden = !selectedStudentId; form.reset(); if (!form.elements.id) form.insertAdjacentHTML('beforeend', '<input type="hidden" name="id">'); Object.entries(student).forEach(([key, value]) => { const field = form.elements[key]; if (!field) return; if (field.type === 'radio') form.querySelectorAll(`[name="${key}"]`).forEach((input) => { input.checked = input.value === value; }); else field.value = value || ''; }); form.elements.retiroId.value = student.retiroId || activeRetreat?.id || ''; form.querySelector('button[type="submit"]').innerHTML = 'Salvar alterações <span>→</span>'; form.querySelector('.delete-student').hidden = false; recalculateBalance(); form.querySelector('#student-message').textContent = 'Cadastro de cursista carregado.'; };
    ...
    editSelectedStudent.addEventListener('click', () => { if (selectedStudentId) { form.scrollIntoView({ behavior: 'smooth', block: 'start' }); form.elements.nome.focus({ preventScroll: true }); } });
