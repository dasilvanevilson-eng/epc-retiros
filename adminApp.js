...
async function renderRelatorios() {
  ...
  const defaultSpec = (datasetId = 'equipe') => {
    ...
    return {
      dataset: dataset.id, statuses: ['concluido'], retreatIds: [], locations: [], periodStart: '', periodEnd: '',
  ...
  const naturalDescription = () => {
    ...
    const locations = spec.locations?.length ? `, em ${spec.locations.map(escapeHtml).join(', ')}` : '';
    const grouping = spec.groupBy.length ? `, agrupado por ${spec.groupBy.map((id) => fieldMap().get(id)?.label).filter(Boolean).join(' e ')}` : '';
    return `${escapeHtml(dataset().label)}, em ${escapeHtml(statuses || 'Encerrado')}, considerando ${retreatText}${locations}${period}${grouping}.`;
  };
  ...
  function wireEvents() {
    ...
    main.querySelectorAll('[data-report-location]').forEach((input) => input.addEventListener('change', () => { remember(); spec.locations = [...main.querySelectorAll('[data-report-location]:checked')].map((item) => item.dataset.reportLocation); report = null; }));
