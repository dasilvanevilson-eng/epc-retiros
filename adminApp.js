    ...
    : studentLinks.length
      ? `<div class="student-registration-link-list">${studentLinks.map((link) => {
        ...
        return `<article class="student-registration-link-row" data-student-link-row="${link.numeroFicha}" data-student-link-registered="${registered ? 'true' : 'false'}">
          <div class="student-registration-link-heading"><div class="student-registration-link-title"><strong>Ficha ${link.numeroFicha}</strong><a class="secondary-button student-registration-link-open" href="${escapeHtml(publicUrl)}" target="_blank" rel="noopener">Abrir</a><label class="student-registration-link-closed"><input type="checkbox" data-student-link-closed="${link.numeroFicha}" ${closed ? 'checked' : ''} ${canEditStudentLinkRecipient ? '' : 'disabled'}><span>Inscrição encerrada</span></label></div><span class="status ${registered || closed ? 'concluido' : 'publicado'}" data-student-link-status="${link.numeroFicha}">${registered ? 'Cadastrada' : (closed ? 'Encerrada' : 'Disponível')}</span></div>
          <span class="student-registration-link-closed-feedback" data-student-link-closed-feedback="${link.numeroFicha}" role="status"></span>
          <div class="student-registration-link-url"><label class="field"><span>Link público — ficha${link.numeroFicha}</span><input value="${escapeHtml(publicUrl)}" readonly aria-label="Link público da ficha ${link.numeroFicha}"></label><button type="button" class="secondary-button" data-copy-student-link="${escapeHtml(publicUrl)}">Copiar link</button></div>
