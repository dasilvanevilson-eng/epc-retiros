...
function wireCepLookup(root) {
  ...
  const lookupCep = async () => {
    ...
    const state = root.elements?.estado || root.elements?.estadoSmp;
...
function coupleStudentRegistrationPrintContent(record, studentFormType) {
  const printRecord = studentFormType === 'cursista-smp' ? {
    ...record,
    outrasUnioesDele: record?.outrasUnioesDele || record?.outrasUnioes || '',
    outrasUnioesDela: record?.outrasUnioesDela || record?.outrasUnioes || '',
  } : record;
  const sharedRows = [
  ...
  const smpOnlyRows = [
    ...
    ['Houve outras uniões?', 'outrasUnioesDele', 'outrasUnioesDela'],
  ...
  const commonFields = studentFormType === 'cursista-epc' ? [
    ...
    ['Data da união', 'uniaoCasal', 'date'], ['Filhos da união', 'filhosUniao'], ['Precisa de acolhimento?', 'precisaAcolhimento'], ['Por que querem fazer o retiro?', 'porqueQueremFazerRetiro'],
    ['Como souberam do retiro?', 'comoSouberamRetiro'], ['Familiar ou amigo', 'familiarAmigo'], ['Fone do familiar', 'foneFamiliar', 'phone'],
  ];
  ...
  <form id="cursista-smp-form" class="panel cursista-smp-form" autocomplete="off">
    ...
    <section class="cursista-smp-section">
      ...
      <div class="fields two-columns">
        ...
        <label class="field"><span>Qual?</span><input name="qualIntoleranciaAlimentarDela" placeholder="Digite a intolerância"></label>
        <fieldset><legend>Precisa de Acolhimento</legend>${yesNo('precisaAcolhimento')}</fieldset>
        <label class="field"><span>Por que querem fazer o retiro?</span><input name="porqueQueremFazerRetiro" placeholder="Digite o motivo"></label>
    ...
    <section class="cursista-smp-section">
      ...
      <div class="fields two-columns">
        <label class="field"><span>Como souberam do retiro?</span><input name="comoSouberamRetiro" placeholder="Digite como souberam do retiro"></label>
...
async function setupCursistaSmpTestCrud({ expectedType = 'cursista-smp', permissionPrefix = 'cursista-smp', label = 'Cursista SMP', initialFileNumber = 0 } = {}) {
  ...
  const loadRecord = (record) => {
    ...
    radioNames.forEach((name) => {
      const legacyValue = ['outrasUnioesDele', 'outrasUnioesDela'].includes(name) ? record.outrasUnioes : '';
      fillRadio(name, record[name] || legacyValue || '');
    });
...
async function renderCrachas() {
  ...
  const syncTextTargetControls = (source = settings) => {
    ...
    form.querySelector('[data-standard-text-color]')?.toggleAttribute('hidden', target === 'name');
