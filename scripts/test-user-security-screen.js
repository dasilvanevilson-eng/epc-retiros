...
assert.match(screen, /permission\.id === 'retiros\.ver' \|\| permission\.id\.startsWith\('links-cadastro\.'\) \? 'Links de cadastro'/);
assert.match(screen, /permission\.id\.startsWith\('retiros\.'\) \? 'Configurações'/);
assert.match(screen, /permissionActionLabels = \{[\s\S]*editar: 'Editar'/);
