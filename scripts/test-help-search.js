...
assert.match(source, /const allowedTargets = new Map\(navItems\.map/, 'A busca deve reutilizar os itens de menu já filtrados por permissão.');
assert.match(source, /\.filter\(\(article\) => allowedTargets\.has\(article\.target\)\)/, 'A ajuda deve mostrar apenas perguntas ligadas a telas permitidas.');
...
assert.match(articles, /target: 'financeiro'[\s\S]*target: 'usuarios'/, 'As respostas devem poder apontar para telas do sistema.');
