...
assert.match(adminSource, /id="student-registration-link-search"[\s\S]*placeholder="Digite o número da ficha"[\s\S]*data-student-link-row="\$\{link\.numeroFicha\}"[^>]*hidden/, 'Os links devem iniciar ocultos e ser localizados pelo número da ficha.');
assert.match(adminSource, /selectStudentLink[\s\S]*rows\.forEach\(\(row\) => \{ row\.hidden = row !== selectedRow; \}\)/, 'A busca deve exibir somente a ficha selecionada.');
assert.match(adminSource, /Nenhuma ficha encontrada com esse número/, 'A busca deve informar quando a ficha não existir.');
