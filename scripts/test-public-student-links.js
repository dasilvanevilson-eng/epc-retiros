...
const publicPage = fs.readFileSync(path.join(root, 'publicStudentRegistrationPage.js'), 'utf8');
assert(!fs.existsSync(path.join(root, 'publicStudentApp.js')), 'A implementação visual pública duplicada deve ser removida.');
...
assert.match(adminSource, /id="student-registration-link-search"[\s\S]*aria-controls="student-registration-link-options"[\s\S]*student-registration-link-options[\s\S]*data-student-link-row="\$\{link\.numeroFicha\}"[^>]*hidden/, 'Os links devem iniciar ocultos e as opções devem ficar vinculadas ao campo de busca.');
assert.match(stylesSource, /\.student-registration-link-options\{position:static;[\s\S]*max-height:min\(65vh,520px\);overflow-y:auto;[\s\S]*scrollbar-gutter:stable/, 'A lista de fichas deve permanecer no fluxo do painel e permitir alcançar as últimas opções pela rolagem.');
assert.match(adminSource, /openStudentLinkOptions[\s\S]*studentLinkSearch\.addEventListener\('focus'[\s\S]*openStudentLinkOptions\(\)/, 'As fichas devem ser exibidas abaixo do campo quando ele receber foco.');
assert.match(adminSource, /data-student-link-option-student><b>Cursista<\/b> \$\{escapeHtml\(link\.nomeCadastrado \|\| ''\)\}[\s\S]*data-student-link-option-recipient><b>Enviado para<\/b> \$\{escapeHtml\(link\.enviadoPara \|\| ''\)\}/, 'Cada opção deve identificar o cursista e o destinatário, mantendo o conteúdo vazio quando ausente.');
...
assert.doesNotMatch(adminSource, />\$\{registered \? 'Cadastrada'/, 'A ficha cadastrada não deve repetir o rótulo Cadastrada no cartão.');
...
assert.match(adminSource, /class="sector-link-feedback student-registration-link-search-feedback"[^>]*role="status">Selecione um setor para visualizar os links\.<\/p>/, 'A orientação da busca por setor deve seguir o mesmo padrão visual da busca por ficha.');
assert.match(adminSource, /sectorLinkSearch\.addEventListener\('input'[\s\S]*!sectorLinkSearch\.value\.trim\(\)[\s\S]*selectedLinks\.innerHTML = ''/, 'Limpar a busca por setor deve remover os links anteriormente selecionados.');
...
assert.match(adminSource, /context\.inscricaoEncerrada[\s\S]*Inscrição encerrada/, 'A página pública compartilhada deve informar o encerramento individual do link.');
