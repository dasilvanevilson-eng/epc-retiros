...
assert.match(ui, /const moneyInputValue = \(value\) => financeNumber\(value\)\.toLocaleString\('pt-BR', \{ minimumFractionDigits: 2, maximumFractionDigits: 2 \}\)/);
assert.match(ui, /data-field="valor" inputmode="decimal" value="\$\{moneyInputValue\(item\.valor\)\}"/);
assert.match(ui, /function formatEventualMoneyInput\(input\)/);
assert.match(ui, /root\.querySelectorAll\('\[data-finance-eventual-row\] \[data-field="valor"\]'\)\.forEach\(formatEventualMoneyInput\)/);
...
assert.match(ui, /\['eventuais', 'Despesas'\]/);
