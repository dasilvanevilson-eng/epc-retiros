...
assert.match(retreatDetailSource, /Acompanhamento do líder<\/span>[\s\S]*data-copy-sector-link="\$\{escapeHtml\(button\.dataset\.followupUrl \|\| ''\)\}"[\s\S]*<a class="secondary-button sector-link-preview-button" href="\$\{escapeHtml\(button\.dataset\.followupUrl \|\| ''\)\}"[\s\S]*>Visualizar<\/a>/, 'O acompanhamento do líder deve oferecer Copiar e Visualizar usando a mesma URL pública.');
assert.doesNotMatch(retreatDetailSource, /Cadastro<\/span>[\s\S]{0,450}data-view-sector-link="\$\{escapeHtml\(button\.dataset\.registrationUrl/, 'O link Cadastro não deve oferecer Visualizar.');
assert.match(retreatDetailSource, /history\.replaceState\(null, '', `#retiros\/\$\{id\}\?setor=\$\{encodeURIComponent\(sector\)\}`\)/, 'Antes de navegar, o sistema deve registrar retiro e setor na rota de retorno.');
assert.doesNotMatch(retreatDetailSource, /sector-link-preview-overlay|sector-link-preview-frame|<iframe/, 'A visualização não deve mais criar sobreposição ou iframe.');
assert.match(source, /const requestedRetreatSector = targetParams\.get\('setor'\) \|\| ''/, 'O roteador deve ler o setor solicitado na rota administrativa.');
assert.match(source, /renderRetreat\(target\.split\('\/'\)\[1\], requestedRetreatSector\)/, 'O retorno deve restaurar o retiro e o setor selecionado.');
assert.match(styles, /\.sector-links-info\{[\s\S]*border-radius:50%/, 'O símbolo de informação dos setores deve ser circular.');
assert.match(styles, /\.sector-link-actions \.sector-link-followup-field\s*\{[\s\S]*grid-template-columns:minmax\(0,1fr\) auto auto/, 'A URL de acompanhamento deve acomodar os botões Copiar e Visualizar lado a lado.');
assert.match(styles, /\.sector-link-followup-field button,[\s\S]*\.sector-link-followup-field a[\s\S]*width:100%/, 'Copiar e Visualizar devem permanecer responsivos em telas pequenas.');
assert.doesNotMatch(styles, /sector-link-preview-overlay|sector-link-preview-dialog|sector-link-preview-frame/, 'Os estilos da antiga sobreposição devem ser removidos.');
