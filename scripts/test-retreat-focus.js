...
assert.match(retreatDetailSource, /Acompanhamento do líder<\/span>[\s\S]*data-copy-sector-link="\$\{escapeHtml\(button\.dataset\.followupUrl \|\| ''\)\}"[\s\S]*data-view-sector-link="\$\{escapeHtml\(button\.dataset\.followupUrl \|\| ''\)\}"[\s\S]*>Visualizar<\/button>/, 'O acompanhamento do líder deve oferecer Copiar e Visualizar usando a mesma URL pública.');
...
assert.match(retreatDetailSource, /sector-link-preview-frame[\s\S]*src="\$\{escapeHtml\(url\)\}"/, 'A sobreposição deve carregar a URL pública em um iframe.');
assert.match(retreatDetailSource, /sector-link-preview-close[\s\S]*trigger\?\.focus\(\)[\s\S]*event\.key === 'Escape'[\s\S]*event\.target === overlay/, 'A prévia deve fechar pelo botão, fundo ou Esc e devolver o foco ao acionador.');
assert.match(retreatDetailSource, /previewFrame\?\.addEventListener\('load'[\s\S]*contentDocument\?\.addEventListener\('keydown', closeFromKeyboard\)/, 'A tecla Esc deve continuar funcionando depois que o foco entrar no iframe.');
...
assert.match(styles, /@media\(max-width:720px\)[\s\S]*\.sector-link-preview-dialog[\s\S]*height:calc\(100vh - 16px\)/, 'A prévia deve ocupar o espaço disponível em telas pequenas.');
