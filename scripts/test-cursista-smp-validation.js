...
async function main() {
  ...
  assert.match(admin, /const focusSmpChurchMarriageIssue = \(form\) => \{[\s\S]*form\.elements\.casamentoDele[\s\S]*form\.elements\.casamentoDela[\s\S]*classList\.add\('field-warning'\)/);
  ...
  assert.match(admin, /const showSmpChurchMarriageDialog = \(form\) => \{[\s\S]*smp-church-marriage-overlay[\s\S]*role', 'dialog'[\s\S]*smpChurchMarriageMessage[\s\S]*Entendi/);
  assert.match(admin, /const close = \(\) => \{[\s\S]*document\.removeEventListener\('keydown', onKeydown\)[\s\S]*overlay\.remove\(\)[\s\S]*focusSmpChurchMarriageIssue\(form\)/);
  assert.match(admin, /message\.textContent = '';[\s\S]*showSmpChurchMarriageDialog\(form\)/);
  ...
  assert.doesNotMatch(styles, /smp-church-marriage-message/);
  ...
  assert.match(admin, /function wirePublicSmpValidation\(form\)[\s\S]*form\.addEventListener\('submit', \(event\) => \{[\s\S]*event\.stopImmediatePropagation\(\)[\s\S]*showSmpChurchMarriageDialog\(form\)[\s\S]*\}, true\)/);
  assert.match(admin, /if \(error\.message === smpChurchMarriageMessage\) \{[\s\S]*showSmpChurchMarriageDialog\(form\)/);
