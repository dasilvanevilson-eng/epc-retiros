...
function balanceSectorHtml(sheet) {
  ...
  return `<section class="panel finance-balance-sector"><h2>${escapeHtml(sheet.setor)}${sheet.historica ? ' <small>(setor histórico)</small>' : ''}</h2>
    ${recurringRows ? `<div class="finance-sheet-scroll"><table class="finance-balance-recurring"><colgroup><col class="finance-balance-description"><col class="finance-balance-unit"><col class="finance-balance-previous"><col class="finance-balance-input"><col class="finance-balance-output"><col class="finance-balance-output-value"><col class="finance-balance-stock"><col class="finance-balance-price"></colgroup><thead><tr><th>Despesa</th><th>Unidade</th><th>Posição anterior</th><th>Entrada</th><th>Saída</th><th>Valor da saída</th><th>Saldo</th><th>Preço unitário</th></tr></thead><tbody>${recurringRows}</tbody></table></div>` : '<p class="empty-state">Sem despesas recorrentes.</p>'}
